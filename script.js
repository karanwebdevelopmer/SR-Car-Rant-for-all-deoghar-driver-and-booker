...
export async function saveDriver(data) {
    ...
    console.log('saveDriver: saved new driver', newRef.key, payload);
    return newRef.key;
}

export async function saveNotification(data) {
    const newRef = push(ref(db, "notifications"));
    const payload = { ...data, createdAt: Date.now(), read: false };
    await set(newRef, payload);
    console.log('saveNotification: saved', newRef.key, payload);
    return newRef.key;
}

export function listenNotifications(callback) {
    onValue(ref(db, "notifications"), snap => callback(snap.val() || {}));
}

export async function updateNotification(id, data) {
    await update(ref(db, `notifications/${id}`), data);
}
