...
function attachWindowHelpers() {
    ...
    window.deleteDriverHandler = async (id) => {
        if (!id) return;
    ...
    container.innerHTML = entries.map(([id, d]) => `
        <div class="card-panel" style="display:flex;flex-direction:column;gap:.85rem">
            ...
            <div>${escapeHtml(d.carDetails || 'No car details')}</div>
                ...
                <button class="btn btn-danger btn-sm" type="button" onclick="deleteDriverHandler('${id}')">Delete</button>
            </div>
        </div>
    `).join('');
}
...
window.approveDriver = async id => {
    ...
    try {
        Object.entries(notifications).forEach(async ([nid, n]) => {
            if (n.driverId === id && !n.read) {
                await updateNotification(nid, { read: true });
            }
        });
    } catch (e) {
...
function renderNotificationsList() {
    ...
    container.innerHTML = sorted.map(([nid, n]) => {
        if (n.type === 'driver_registration') {
            return `
        <div class="card-panel" style="margin-bottom:1rem">
            <div style="display:flex;justify-content:space-between;flex-wrap:wrap;gap:1rem">
                <div>
                    <strong>${escapeHtml(n.name)}</strong><br>
                    <small>Driver registration · ${escapeHtml(n.mobile)}</small>
                    <div style="margin-top:.5rem;color:var(--muted-text);font-size:.85rem">${formatDate(n.createdAt)}</div>
                </div>
                <div style="display:flex;gap:.5rem;align-items:center">
                    <button class="btn btn-primary btn-sm" type="button" onclick="approveDriver('${n.driverId}')">Approve Now</button>
                    ${n.read ? '<span class="badge badge-success">Read</span>' : '<span class="badge badge-warning">New</span>'}
                </div>
            </div>
        </div>
    `;
        ...
        <div class="card-panel" style="margin-bottom:1rem">
            <div style="display:flex;justify-content:space-between;flex-wrap:wrap;gap:1rem">
                <div>
                    <strong>${escapeHtml(n.type || 'Notification')}</strong><br>
